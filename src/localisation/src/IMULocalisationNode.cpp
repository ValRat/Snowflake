...
void IMULocalisationNode::subscriberCallBack(const sensor_msgs::Imu::ConstPtr& imu_msg) {
    ...
    if (initial_orientation == nullptr){
        // Convert our geometry_msgs::Quaternion to a tf::Quaternion
        initial_orientation = tf::createIdentityQuaternion();
        tf::quaternionMsgToTF(imu_msg->orientation, initial_orientation);
...
void IMULocalisationNode::updateCurrPosition(sensor_msgs::Imu imu_msg) {
    ...
    // Convert our received orientation to a tf::Quaternion
    tf::Quaternion tf_curr_orientation;
    tf::quaternionMsgToTF(imu_msg.orientation, tf_curr_orientation);
    // The acceleration we receive from the IMU is relative to where the IMU is pointing,
    // so translate to our initial orientation
    tf::Quaternion translated_orientation = tf_curr_orientation * initial_orientation.inverse();
    // Convert our acceleration to a tf::Vector3
    tf::Vector3 curr_acceleration;
    tf::vector3MsgToTF(imu_msg.linear_acceleration, curr_acceleration);

    // YOU ARE HERE: Need to deal with all the type conversions to feed your translated acceleration
    // into the rest of your functions
    geometry_msgs::Vector3& curr_acceleration = tf::quatRotate(translated_orientation, curr_acceleration);
    geometry_msgs::Vector3 curr_velocity = calcNewVelocity(prev_velocity, curr_acceleration, prev_acceleration, delta_time);

    // Update our state
    prev_position = calcNewPosition(prev_position, prev_velocity, curr_velocity, delta_time);
    prev_acceleration = imu_msg.linear_acceleration;
...
double IMULocalisationNode::calcPositionChange1D(double velocity, double curr_velocity, double delta_time) {
