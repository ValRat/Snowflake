...
void VisionDecision::imageCallBack(const sensor_msgs::Image::ConstPtr &image_scan) {
    ...
    decision::TwistConfidence twistMsgWithConfidence;
    twistMsgWithConfidence.confidence = confidence_value;
    // Publish the twist message
    publishTwist(twistMsgWithConfidence);
}

void VisionDecision::publishTwist(decision::TwistConfidence twistConfidence) {
    twistConfidence.header.stamp = ros::Time::now();
    twist_publisher.publish(twistConfidence);
}
