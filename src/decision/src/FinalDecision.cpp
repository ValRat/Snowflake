...
FinalDecision::FinalDecision(int argc, char **argv, std::string node_name) {
    ...
    int refresh_rate = 10;
    lidar_subscriber = private_nh.subscribe(lidar_decision_topic_name, refresh_rate, &FinalDecision::lidarCallBack,
                                            this);
    vision_subscriber = private_nh.subscribe(vision_decision_topic_name, refresh_rate, &FinalDecision::visionCallBack,
                                             this);
...
void FinalDecision::arbitrator(const ros::TimerEvent &timerEvent) {
    ...
    if (recent_lidar.confidence > 70 && recent_vision.confidence > 70) {
        recent_vision.twist.angular.z = 1.5 * recent_vision.twist.angular.z;
        recent_vision.twist.linear.x = -0.5 * recent_vision.twist.linear.x;
        publishTwist(recent_vision.twist);
    } else if (recent_lidar.confidence > recent_vision.confidence && recent_lidar.confidence > recent_gps.confidence)
        publishTwist(recent_lidar.twist);
    else if (recent_vision.confidence > recent_lidar.confidence && recent_vision.confidence > recent_gps.confidence)
        publishTwist(recent_vision.twist);
    else {
...
void FinalDecision::updateConfidence(decision::TwistConfidence &twistConfidence) {
    if ((ros::Time::now() - twistConfidence.header.stamp) > old_message_threshold) {
        ...
        if (twistConfidence.confidence < 0) twistConfidence.confidence = 0;
    }
}
